################################################################################
#                                                                              #
# Original Authors for this code: Jean Kossaifi, Antoine Toisoul, Adrian Bulat #
#                                                                              #
################################################################################
# This code can be found at https://github.com/face-analysis/emonet
# This code is licensed under the CC BY-NC-ND License (see LICENSE from the emonet for details)
# This code has been modified from the original to fit the needs of this project.
...
class EmoNet(nn.Module):
    ...
    def forward(self, x, reset_smoothing=False):
        ...
        final_features1 = self.emo_fc_2(final_features)

        if self.temporal_smoothing:
            with torch.no_grad():
                self.temporal_state[:, :-1, :] = self.temporal_state[:, 1:, :]
                self.temporal_state[:, -1, :] = final_features1
                final_features1 = torch.sum(self.temporal_weights * self.temporal_state, dim=1)

        return {'heatmap': tmp_out, 'expression': final_features1[:, :-2], 'valence': final_features1[:, -2],
                'arousal': final_features1[:, -1], 'feature': final_features}
