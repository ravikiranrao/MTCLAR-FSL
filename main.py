...
from src.utils import load_config, SaveBestModel, save_model, dump_config, EarlyStopping,\
    conf_mat, get_logger, get_tensorboard_writer, VisdomLinePlotter, image_pairs_classification_viz
from src.data import get_dataset
from src.model import get_model
from src.tools import train, test, get_scheduler, train_cos_embed, test_cos_embed
...
save_dir = os.path.join("Experiments_New2", datetime.datetime.now().strftime("%Y%m%d-%H%M%S") + "_" + cfg['EXPERIMENT_NAME'])
...
# Create Tensorboard Writer and save it
writer = get_tensorboard_writer(os.path.join(save_dir))
global plotter
...
'''
image_pairs_classification_viz(y_true=test_dict['target_true'],
                               y_pred=test_dict['target_pred'],
                               categories_pair=test_dict['categories_pair'],
                               display_labels=["Neutral", "Happy", "Sad", "Surprise", "Fear",
                                               "Disgust", "Angry", "Contempt"],
                               savefig_path=os.path.join(save_dir, 'emotion_pair_plot.png'),
                               savefig_path_symm=os.path.join(save_dir, 'emotion_pair_plot_symm.png'),
                               savefig_path_sim=os.path.join(save_dir, 'emotion_pair_plot_sim.png'),
                               title='Similarity/Dissimilarity Accuracy for each pair',
                               title_symm='Similarity/Dissimilarity Accuracy for each pair (symmetry assumption)',
                               title_sim='Similarity Accuracy for each pair'
                               )
'''
